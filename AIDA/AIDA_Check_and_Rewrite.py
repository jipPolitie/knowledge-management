...
# make this absolute to project folder, but not to machine absolute
...
not_atomic_list = ["and that", "and also", "but ", "so that", "while ", "however ", "whereas ", "on the other hand",
...
not_absolute_list = ["probably", "perhaps", "potentially", "putative", "maybe", "plausible", "possible", "likely",
                     "feasible", "hypothetical", "may", "could ", " seem ", "appears to", "appear to", " appear ",
                     " might ", " suggest ", "minimally sufficient", "is predicted", "is foreseen", "is envisioned",
                     "revealed that", "reveals that", "significant", "significantly", "to reveal", " estimated ",
                     " estimate"]
    ...
    for prediction in predictions:
        if prediction in sentence:
            for i, tag in enumerate(tagged):
                if tag[0] == "predicted" or tag[0] == "foreseen" or tag[0] == "envisioned" and tagged[i - 1][
                    0] == "is" and (tagged[i - 2][1] == "NNP" or tagged[i - 2][1] == "NN"):
...
for sentence in sentences:
    ...
    writer.writerow({'Sentence': sentence, 'Atomic': Atomic, 'Independent': Independent, 'Declarative': Declarative,
                     'Absolute': Absolute, 'AIDA': AIDA, 'Rewritten_Sentence': rewritten_sentence})
